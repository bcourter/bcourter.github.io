...
Blake Courter, with contributions from [Luke Church](https://www.linkedin.com/in/luke-church/), [Dan Rubel](https://www.linkedin.com/in/danrubel/), [Keegan McNamara](https://www.linkedin.com/in/keegan-mcnamara/), and the Gradient Control team and extended family.  
...
Within six months of starting my career at [PTC](https://www.ptc.com/) three decades ago, I saw the problem. I had thought that by selling Pro/E, I'd survey the engineering disciplines and learn how engineers design. Instead, I learned what drafters do: create technical drawings that get passed downstream for manufacturing and archival. The actual engineering knowledge, the *why* behind the design, was nowhere to be found. Where did it live?

Thirty years later, we're still searching for this mystical "design intent." Our most sophisticated 3D models are still just collections of lines and curves with no understanding of their purpose. They're documentation tools, not intelligent systems. Engineering knowledge remains inaccessible in Matlab and Python scripts, FEA datasets that no longer run in the latest codes, a diaspora of Office documents, and possibly the tombs of PLM.

The team at Cursor reapplied their LLM tech to code and built one of the most successful developer tools in recent memory. If we could represent geometry as code, could we perhaps achieve a Cursor for CAD?
...
Additionally, these signed functions are equivalent to *machine learning classifiers*. In the same way that a classifier determines whether a picture contains or does not contain an object like a hot dog. We're just classifying space as "part" or "not part." Modern data science provides amazing tools to work on geometry as code!

Not only can we tell where the part is, we can tell how it relates to other parts by comparing the fields. We can add automatic differentiation, tolerance stack-up, and topological fields to harmonize previously unconnected disciplines. To evaluate fitness, we can bring AI workhorses such as [Monte Carlo techniques](https://www.cs.cmu.edu/~kmcrane/Projects/WalkOnStars/) and [quadrature](https://intact-solutions.com/) to the table to build fully differentiable engineering models.
...
This isn't a new idea, but only recently have computers become powerful enough to make it practical:
...
- **Version control** for distributed engineering teams
- **Unprecedented scaling** from quantum to Lorentz space (as long as you don't do both at the same time)
...
Meanwhile, [Dan Rubel](https://www.linkedin.com/in/danrubel/) treated LatticeRobot as a training mission and used our Alpha approach SaaS architecture, now well exercised in [Rapid Liquid Print's VEER ON](https://www.rapidliquidprint.com), to host a new compiler for a refined language along with a WebGPU front end and Fidget instead of libfive for meshing, etc.
