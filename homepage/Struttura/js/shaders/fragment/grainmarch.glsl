...
vec3 getLight(in vec3 color, in vec3 normal, in vec3 dir) {
	...
	vec3 c1 = vec3(1.0, 0.0, 0.2);
	vec3 c2 = vec3(0.1, 0.0, 1.0);

	return
	(diffuse*Diffuse)*(Color1*color) +
	(diffuse2*Diffuse)*(Color2*color);
}
...
void main(void)
{
    ...
    vec2 res = vec2(100.0, 80.0);
    
	vec2 coord =-1.0+2.0*vVertexPosition.xy/res.xy;
	coord.x *= res.x/res.y;
