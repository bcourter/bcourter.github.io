...
float LogOverexpose = pow(2.0, Overexpose);
float LogStripePeriod = pow(10.0, StripePeriod);
...
void main(void)
{
    ...
    vec2 z0 = rotate(position + vec2(2.0 * (ShiftX + shirtX) - 1.0, 2.0 * (ShiftY + shirtY) - 1.0) - vec2(0.5, 0.5)) * 2.0 * domain;
	vec2 fz0 = f(z0);
    vec2 fzd = f(z0 + t * w);
    vec2 fk = (fzd - fz0) / (t * w);
    //vec2 fk = fz0;
